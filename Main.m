...
config.RUN_TRANSFORM_ANALYSIS = 0;
...
% Display data on command line: T - tumor cells, O - normal cells.
[NumCancerCells, NumNormalCells, cell_samples, SampleSize, data_loaded, Y] = ...
    LoadData(config);
    
% Perform transform domain analysis of the three-step values.
step_values = AnalysisWHTStepValues(config, NumCancerCells, ...
    NumNormalCells, data_loaded);
...
if config.RUN_WHT_CLASSIFICATION == 1
    ...
    [Accuracy_SVM, Accuracy_kNN, Accuracy_NB, Accuracy_DT, ...
        Accuracy_AdaBoost, Accuracy_LogitBoost, Accuracy_Bagging, ...
        Accuracy_Subspace, Accuracy_LinearModel, Accuracy_NN, ...
        Accuracy_LSTM, time_record] = ClassifiersWHTDomain(config, ...
        cell_samples, SampleSize, data_loaded, Y);
